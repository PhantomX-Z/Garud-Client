...
public:
	...
	std::string skyt[4] = {"Galaxy", "Tropical Sunnyday", "Cosmos", "Planet X"};
	void autoaim(v3f origin, f32 max_d);
...
void Game::run()
{
	...
	while (RenderingEngine::run()
        ...
        // These functions are being called every frame because it just makes them work better.
		core::line3d<f32> shootline = getShootline();
		if (g_settings->getBool("Killaura")) {
			handleKillaura(shootline.start, shootline.getLength());
		}
		if (g_settings->getBool("autoaim")) {
			autoaim(shootline.start, shootline.getLength());
		}
...
void Game::handleClientEvent_SetSky(ClientEvent *event, CameraOrientation *cam)
{
	...
	if (g_settings->getBool("custom_sky")) {
...
core::line3d<f32> Game::getShootline() {
    f32 d = g_settings->getFloat("reachd");
    ...
    if (g_touchscreengui && g_touchscreengui->isActive() && m_cache_touchtarget) {
        ...
        shootline.end = shootline.start + shootline.getVector().normalize() * BS * d;
...
void Game::drawHUD(video::IVideoDriver *driver, double dtime)
{
	gui::IGUIFont *m_font = g_fontengine->getFont(FONT_SIZE_UNSPECIFIED, FM_Standard);
	...
	for (auto category = script->m_cheat_categories.begin();
        ...
        for (auto cheat = (*category)->m_cheats.begin();
            ...
            if ((*cheat)->is_enabled()) { 
				if ((*cheat)->m_name == "Custom Sky") {
					(*cheat)->m_name = std::string("Custom Sky [ ") + skyt[(int)(g_settings->getFloat("skytexture") - 1)] + " ]";
...
double getRandomInt(double min, double max) {
    ...
    double biased_value = (random * random) * (max - min) + min;

    return (distribution(generator) < 0.5) ? -biased_value : biased_value;
}
...
void Game::autoaim(v3f origin, f32 max_d) 
{

    if (client->getHP() <= 0 || isKeyDown(KeyType::SNEAK)) {
        return;
    }
		...
		const ItemStack &tool_item, const v3f &player_position, bool show_debug)
{
	...
	if (should_punch || g_settings->getBool("autohit")) {
		bool do_punch = false;
		...
		const CameraOrientation &cam)
{
	...
	*/
	...
	RenderingEngine::draw_scene(skycolor, m_game_ui->m_flags.show_hud,
			m_game_ui->m_flags.show_minimap, draw_wield_tool, draw_crosshair, g_settings->getBool("node_esp"), g_settings->getBool("node_tracers"), g_settings->getBool("entity_esp"), g_settings->getBool("entity_tracers"), g_settings->getBool("player_esp"), g_settings->getBool("player_tracers"));
