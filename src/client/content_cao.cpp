...
bool GenericCAO::isPlayerFriendly(GenericCAO *playerObj)
{
...
void GenericCAO::processMessage(const std::string &data)
{
	...
	if (cmd == AO_CMD_SET_PROPERTIES) {
		...
		g_settings->setFloat("ovspeed", override_speed);
		g_settings->setFloat("ovjump", override_jump);
		g_settings->setFloat("ovgravity", override_gravity);
		g_settings->setBool("ovsneak", sneak);
		g_settings->setBool("ovsneak_glitch", sneak_glitch);
		g_settings->setBool("ovnewmove", new_move);
		if(m_is_local_player && !g_settings->getBool("customphysics"))
		{
