			...
			RenderingEngine::get_scene_manager(), id)
{
	...
	setStarCount(g_settings->getU16("Star_Count"),
		     true);
}
...
void Sky::render()
{
	...
	if (m_sunlight_seen) {
		...
		if(g_settings->getBool("No_Stars") || m_in_clouds)
 			return;
  	...
  	// Draw the six sided skybox,
		if (m_sky_params.textures.size() == 6) {
			for (u32 j = 5; j < 11; j++) {
				...
				// sun, moon and stars, as this is a background mskybox.
		...
		if (m_sun_params.sunrise_visible && g_settings->getBool("custom_sky") == false) {
